...
import root.iv.protection.util.Action1;
import root.iv.protection.util.Signed;
...
public class DES  {
    ...
    public int[] cipher(int[] input) {
        ...
        for (int b = 0; b < blocks.length; b++) {
            ...
            blocks[b].transfer(Const.IP);

            // 16 Раундов шифрования
            for (int i = 0; i < 16; i++) {
                blocks[b].value = roundCipher(blocks[b].value, i);
            }

            // Конечная перестановка
            blocks[b].transfer(Const.IP_1);
        }
    ...
    public int[] decipher(int[] input) {
        ...
        for (int b = 0; b < blocks.length; b++) {
            ...
            for (int i = 15; i >=0; i--) {
                blocks[b].value = roundDecipher(blocks[b].value, i);
            }
